# 📌 PEX 05 — Instituição Aurora (Relato Final e Apresentação)

## 📖 Introdução

O Projeto de Extensão V representa a etapa final do desenvolvimento das atividades realizadas para a Instituição Aurora – Centro de Acolhimento Emocional (fictícia). Nesta fase, são apresentados os resultados obtidos a partir das soluções propostas nas etapas anteriores, bem como uma reflexão sobre os impactos gerados.

---

## 💻 Apresentação da Solução

Como solução para os problemas identificados, foi desenvolvido o portal web **Aprenda Fácil**, com o objetivo de organizar informações, disponibilizar conteúdos de apoio emocional e facilitar o acesso dos usuários aos serviços da instituição.

---

## 📸 Evidências da Solução

Para validar a proposta desenvolvida, foi criado o portal web **Aprenda Fácil**, que pode ser acessado no link abaixo:

🔗 https://amanda-aandrade.github.io/aprenda-facil-pex/

A plataforma apresenta funcionalidades completas voltadas ao apoio emocional e organização de conteúdos, incluindo:

- Página inicial com navegação intuitiva
- Área de cursos educativos
- Biblioteca de e-books
- Guia de apoio emocional
- Estrutura de perfil do usuário

Mesmo sendo um projeto acadêmico, a solução foi desenvolvida com base em cenários reais, simulando a utilização por usuários que necessitam de apoio emocional e acesso facilitado à informação.

---

## 🧪 Simulação de Uso (Validação)

Para demonstrar a aplicabilidade do sistema, foi considerada a seguinte situação simulada:

Um usuário acessa o portal em busca de orientação emocional. A partir da página inicial, ele consegue navegar até o guia de apoio, acessar conteúdos educativos e utilizar os recursos disponíveis de forma simples e organizada.

Essa simulação demonstra que o sistema é funcional, acessível e capaz de atender às necessidades identificadas na Instituição Aurora.

---

🔗 Acesse o projeto:  
https://amanda-aandrade.github.io/aprenda-facil-pex/

O sistema conta com as seguintes funcionalidades:

- Página inicial (index)
- Área de cursos
- E-books educativos
- Guia de apoio emocional
- Página sobre o projeto
- Perfil do usuário

O desenvolvimento foi realizado utilizando tecnologias como **HTML, CSS e JavaScript**, garantindo uma interface acessível, simples e funcional.

---

## 🏥 Contribuição para a Instituição Aurora

A solução proposta contribui para a melhoria dos processos internos da instituição, proporcionando:

- Centralização das informações
- Melhoria na comunicação com os usuários
- Facilidade de acesso aos conteúdos
- Redução de processos manuais
- Apoio ao acompanhamento dos atendimentos

---

## 🎓 Relação com o Curso

O desenvolvimento do projeto permitiu aplicar conhecimentos adquiridos no curso de Análise e Desenvolvimento de Sistemas, tais como:

- Desenvolvimento web
- Organização de sistemas
- Estruturação de informações
- Lógica de programação
- Resolução de problemas

Essa experiência contribuiu para o desenvolvimento profissional e prático da autora.

---

## 🌍 Impacto Social e ODS

O projeto está alinhado ao **ODS 3 — Saúde e Bem-estar**, ao promover o acesso à informação e incentivar o cuidado com a saúde emocional.

Mesmo sendo uma simulação acadêmica, o projeto demonstra como a tecnologia pode ser utilizada para gerar impacto positivo na sociedade.

---

## 📝 Considerações Finais

A realização do PEX V possibilitou consolidar os conhecimentos adquiridos ao longo do curso, além de evidenciar a importância da tecnologia como ferramenta de transformação social.

O portal **Aprenda Fácil** representa uma solução viável e escalável, capaz de contribuir para a melhoria dos serviços prestados pela Instituição Aurora.

Quero agradecer a Faculdade Descomplica por proporcionar oportunidades incríveis e disponibilizar métodos de estudos que atendem todos os seus alunos. Em cada aula assistida e em cada 
momento de desespero por medo de não atingir as metas, a Descomplica foi sensacional em prestar apoio. E também, quero agradecer a minha esposa que sempre me apoiou em cada momento 
nessa transição de carreira e trajetória incrível. Obrigada por tanto! 

---

## 👩‍💻 Autora

Amanda Anáalia Da S. Andrade  
Curso: Análise e Desenvolvimento de Sistemas RA: 2416820
